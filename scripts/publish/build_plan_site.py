...
<body>
  ...
  <main>{body}</main>
  <footer class="site-footer"><div class="shell footer-grid"><div class="footer-copy"><a class="brand" href="{site_url}/"><img src="{asset_root}/dersdefter-logo.png" alt="" width="48" height="48">DersDefter</a><p>Öğretmenin haftalık ders hazırlığını tek yerde toplayan iPhone ve Android uygulaması.</p></div><nav class="footer-links" aria-label="Alt menü"><a href="{site_url}/#planlar">Planlara göz at</a><a href="{APP_STORE_URL}">App Store</a><a href="{PLAY_STORE_URL}">Google Play</a><a href="{site_url}/llms.txt">LLM bilgisi</a></nav></div></footer>
</body></html>
"""
...
<section class="section mint" id="planlar"><div class="shell"><div class="section-head"><div class="eyebrow">{html.escape(period)} yıllık planları</div><h2>Öne çıkan derslere göz atın.</h2><p>Dersiniz burada görünmüyorsa DersDefter’i açın. {format_number(len(entries))} sınıf–ders başlığının tamamı uygulamada yer alır.</p></div><div class="catalog-wrap"><div class="result-meta"><span>Öne çıkan planlar</span><span>Haftalık ayrıntılar yalnız uygulamada açılır.</span></div><div class="plan-grid">{cards}</div><div class="actions"><a class="button" href="{APP_STORE_URL}">App Store’dan indir</a><a class="button alt" href="{PLAY_STORE_URL}">Google Play’den indir</a></div></div></div></section>
...
def build_site(index_path: Path, output_root: Path, site_url: str) -> list[dict[str, object]]:
    ...
    public_catalog = output_root / "plan-index.json"
    if public_catalog.exists():
        public_catalog.unlink()
